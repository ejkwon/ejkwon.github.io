...
function gui_row_select(id, setfocus, multiple) {
	...
	props[id].style.background = '#e1e1e1';
	//props[id].style.width = "733px"
}
...
function gui_input_change(e) {
	...
	if (obj.name == 'img_href')        {myimgmap.areas[id].ahref   = obj.value;}
		...
		if (myimgmap.areas[id].shape != obj.value && myimgmap.areas[id].shape != 'undefined') {
			...
			var coords = '';
			if (props[id]) {
				coords  =  props[id].getElementsByTagName('input')[3].value;
			}
			else {
			...
			if (props[id]) {
				props[id].getElementsByTagName('input')[3].value  = coords;
			}
	...
	if (myimgmap.areas[id] && myimgmap.areas[id].shape != 'undefined') {
		myimgmap._recalculate(id, props[id].getElementsByTagName('input')[3].value);
...
function gui_addArea(id) {
	...
	temp+= '<input type="text" name="img_href" class="img_href" value="" placeholder="랜딩(href)">';
	temp+= '<input type="text" name="img_coords" class="img_coords" value="" placeholder="Coords">';
	...
	temp+= '</select>';
	temp+= '<div style="display:none;">Alt: <input type="text" name="img_alt" class="img_alt" value=""></div>';
	temp+= '<div style="display:none;">Target: <select name="img_target" class="img_target">';
...

function textAddEvent(str) {
	var imgurl = document.getElementById('source_url2').value;
	var mapid = myimgmap.mapid;
	var tagStr = "";

	if(radionum == 1){
        tagStr += '<img src="'+imgurl+'" alt="" border="0" usemap="#'+mapid+'" />\n';
		tagStr += str+'\n';
    } else if (radionum == 2) { //랜딩
		tagStr = '<html lang="ko">\n';
		tagStr += '<head>\n';
		tagStr += '    <meta http-equiv="Content-Type" content="text/html; charset=utf-8">\n';
		tagStr += '    <title>cafe24</title>\n';
		tagStr += '</head>\n';
		tagStr += '<body>\n';
		tagStr += '<span style="text-align:center;">\n';
		tagStr += '    <img src="'+imgurl+'" alt="" border="0" usemap="#'+mapid+'" />\n';
		tagStr += '</span>\n';
		tagStr += str+'\n';
		tagStr += '</body>\n';
		...
		tagStr += '</head>\n';
		tagStr += '<body>\n';
		tagStr += '<table width="690" border="0" cellpadding="0" cellspacing="0" align="center" style="font-family:Dotum;">\n';
		tagStr += '    <tr>\n';
		tagStr += '        <td style="padding:0; font-size:0; line-height:0;">\n';
		if(radionum == 3){
			tagStr += '            <img src="http://m-img.cafe24.com/images/template/hosting/img_header.gif" alt="cafe24 호스팅 센터" usemap="#logoMap" border="0">\n';
			tagStr += '            <map name="logoMap">\n';
			tagStr += '                <area shape="rect" coords="32,21,260,53" href="http://hosting.cafe24.com" target="_blank" alt="호스팅 센터">\n';
			tagStr += '            </map>\n';
		}else if(radionum == 4){
			tagStr += '            <img src="http://m-img.cafe24.com/images/template/ec/img_header.gif" alt="cafe24 쇼핑몰 센터" usemap="#logoMap" border="0">\n';
			tagStr += '            <map name="logoMap">\n';
			tagStr += '                <area shape="rect" coords="32,21,260,53" href="http://echosting.cafe24.com" target="_blank" alt="쇼핑몰 센터">\n';
			tagStr += '            </map>\n';
		}else if(radionum == 5){
			tagStr += '            <img src="http://m-img.cafe24.com/images/template/soho/img_header.gif" alt="cafe24 창업센터" usemap="#logoMap" border="0">\n';
			tagStr += '            <map name="logoMap">\n';
			tagStr += '                <area shape="rect" coords="32,21,260,53" href="http://soho.cafe24.com/" target="_blank" alt="창업센터">\n';
			tagStr += '            </map>\n';
		}else if(radionum == 6){
			tagStr += '            <img src="http://m-img.cafe24.com/images/template/cmc/img_header.gif" alt="cafe24 마케팅센터" usemap="#logoMap" border="0">\n';
			tagStr += '            <map name="logoMap">\n';
			tagStr += '                <area shape="rect" coords="32,21,260,53" href="http://cmc.cafe24.com/" target="_blank" alt="마케팅센터">\n';
			tagStr += '            </map>\n';
		}
		tagStr += '        </td>\n';
		tagStr += '    </tr>\n';
		tagStr += '    <tr>\n';
		tagStr += '        <td style="padding:25px 0 0; border-left:1px solid #d8d8d8; border-right:1px solid #d8d8d8; text-align:center; font-size:0; line-height:0;">\n';
		tagStr += '            <img src="'+imgurl+'" alt="" usemap="#'+mapid+'" border="0" />\n';
		tagStr += str+'\n';
		tagStr += '        </td>\n';
		tagStr += '    </tr>\n';
		tagStr += '    <tr>\n';
		tagStr += '        <td style="padding:0; font-size:0;">\n';
		tagStr += '            <img src="http://m-img.cafe24.com/images/template/ec/img_footer_top.gif" alt="" border="0">\n';
		tagStr += '        </td>\n';
		tagStr += '    </tr>\n';
		tagStr += '    <tr>\n';
		tagStr += '        <td style="font-size:0; line-height:0; background-color:#f4f4f4;">\n';
		tagStr += '            <table width="100%" border="0" cellpadding="0" cellspacing="0">\n';
		tagStr += '                <tr>\n';
		tagStr += '                    <td width="173" style="vertical-align:middle; text-align:center;">\n';
		tagStr += '                        <a href="http://www.cafe24.com" target="_blank"><img src="http://m-img.cafe24.com/images/template/ec/txt_footer_bi.gif" alt="cafe24" border="0"/></a>\n';
		tagStr += '                    </td>\n';
		tagStr += '                    <td width="493">\n';
		tagStr += '                        <table border="0" cellpadding="0" cellspacing="0">\n';
		tagStr += '                            <tr>\n';
		tagStr += '                                <td width="530" style="padding:25px 0 15px 0; color:#8a8e94; font-size:11px; font-family:dotum,'+"돋움"+','+"돋움체"+'; line-height:18px;">\n';
		tagStr += '                                    본 메일은 0000년 00월 00일 기준으로 회원님의 카페24 이메일 수신동의 여부를 확인한 결과,<br />수신에 동의하였기에 발송되었습니다. <br />메일 수신을 원하지 않으시면 <a href="https://user.cafe24.com/unsubscribe/?action=unsubscribeAgreement&userId=[$memberid]&email=[$etc1]"><img src="http://m-img.cafe24.com/images/webzine/center/btn_deny.gif" alt="수신거부" border="0" style="vertical-align: middle;"></a> 를 클릭해 주십시오.<br /><br />If you do not want to receive emails from us, please click here. [<a href="https://user.cafe24.com/unsubscribe/?action=unsubscribeAgreement&userId=[$memberid]&email=[$etc1]" style="text-decoration:none; font-weight:bold; color:#8a8e94; ">HERE</a>]<br />본 메일은 발신전용으로 회신되지 않으므로 문의사항은 [고객센터]를 이용하여 주시기 바랍니다.\n';
		tagStr += '                                </td>\n';
		tagStr += '                            </tr>\n';
		tagStr += '                            <tr>\n';
		tagStr += '                                <td style="padding:15px 0 13px; border-top:1px solid #d7d7d7; font-size:11px; line-height:18px; color:#8a8e94; font-family:Tahoma,Dotum;"><strong style="color:#67696b; font-size:12px;">카페24</strong>(주) <span style="color:#c3c3c3;">|</span> 대표이사 : 이재석<br />주소 : 서울특별시 동작구 보라매로5길 15(신대방동, 전문건설회관)<br />사업자등록번호 : 118-81-20586 <span style="color:#c3c3c3;">|</span> 통신판매업신고 : 동작 제 02-680-078호 <br />전화번호 : 1588-3284 &nbsp; 팩스 : 02-6280-0331 &nbsp;전자우편 : <a href="mailto:echosting@cafe24.com" style="color:#8a8e94; text-decoration:none;">echosting@cafe24.com</a></td>\n';
		tagStr += '                            </tr>\n';
		tagStr += '                            <tr>\n';
		tagStr += '                                <td style="padding:15px 0 0;"><img src="http://m-img.cafe24.com/images/template/ec/txt_footer_copy.gif" alt="Copyright &copy; Cafe24 Corp. All Rights Reserved." border="0" /></td>\n';
		tagStr += '                            </tr>\n';
		tagStr += '                        </table>\n';
		tagStr += '                    </td>\n';
		tagStr += '                    <td width="24">&nbsp;</td>\n';
		tagStr += '                </tr>\n';
		tagStr += '            </table>\n';
		tagStr += '        </td>\n';
		tagStr += '    </tr>\n';
		tagStr += '    <tr>\n';
		tagStr += '        <td colspan="2">\n';
		tagStr += '            <img src="http://m-img.cafe24.com/images/template/ec/img_footer_bottom.gif" alt="" border="0" />\n';
		tagStr += '        </td>\n';
		tagStr += '    </tr>\n';
		tagStr += '</table>\n';
		tagStr += '</body>\n';
	...
	document.getElementById('html_container').value = tagStr;
}

function radiocheck(num){
	radionum = num;

	var str = myimgmap.getMapHTML();
	textAddEvent(str);
	//var review = document.getElementById('html_container').value;
	//document.getElementById('pic_container').innerHTML = review;
	if(radionum == 1 || radionum == 2){
		document.getElementById("mailblock").style.display = "none"
	}
}

function mailblockview(){
	radiocheck(3);
...
function htmlCreateEvent(){
	var newHtml = document.getElementById("html_container").value;

	var fileName = "";
	if(radionum == 1){
		fileName = "event.html";
	}else{
		fileName = "event_mail.html";
	}
	saveAs(newHtml, fileName);
}

function saveAs(newHtml, fileName){
	var agent = navigator.userAgent;
	if(agent.indexOf('Trident')>0){				//ie �϶�
		console.log("ie");
		document.getElementById('iframe_target').contentWindow.document.open("text/html","replace");
		document.getElementById('iframe_target').contentWindow.document.write(newHtml);
		document.getElementById('iframe_target').contentWindow.document.close();
		document.getElementById('iframe_target').focus();
		document.getElementById('iframe_target').contentWindow.document.execCommand('SaveAs', null, fileName);
	}else{			//����, ũ�� �϶�
		console.log("ff, ch");
		var element = document.createElement('a');
		element.setAttribute('href', 'data:text/plain;charset=utf-8,' + encodeURIComponent(newHtml));
		...
		/*
			var html = document.getElementById('iframe_target').contentWindow.document.documentElement.innerHTML;
			if(agent.indexOf('Firefox')>0 && window.btoa){
				...
				var type = 'text/html'
				var a = document.getElementById('iframe_target').contentWindow.document.createElement('a');
				a.download = fileName;
...
function gui_areaChanged(area) {
	...
	if (props[id]) {
		...
		if (area.lastInput) {props[id].getElementsByTagName('input')[3].value  = area.lastInput;}
		if (area.ahref)   {props[id].getElementsByTagName('input')[2].value  = area.ahref;}
