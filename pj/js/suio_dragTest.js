...
var SUIO = function(){
   ...
   // mLayer : close
    $.mLayer_close = function(target){
		...
		var findClass = findParent.hasClass('eLayerDrag');
		if(findClass){
			$('.eLayerDrag').draggable('destroy');
			findParent.removeClass('eLayerDrag');
			findParent.removeAttr("style");
		}
        return false
    }
    ...
    $('body').delegate('.eLayerClick', 'click', function(e){
        ...
        //eLayerClick Draggable
        findTarget.addClass('eLayerDrag');
        $('.eLayerDrag').draggable({ cursor: "move", cancel:'.wrap, .footer' });

        e.preventDefault();
    });
    
    //eLayerClick Draggable
	$('body').delegate('.eLayerDrag h2', 'mouseover', function(e){
		$(this).css({'cursor':'move'});
	});
	$('body').delegate('.eLayerDrag h2', 'mouseout', function(e){
