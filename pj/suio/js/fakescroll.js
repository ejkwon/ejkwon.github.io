...
//animate, useswipe, tag를 제외한 옵션은 x, y를 구분해서 따로 사용가능
fs.defaultoption={
	...
	//스크롤 될 때 wrapper에 css transform을 사용할 것인지 여부(true 또는 false).
	//지원하는 브라우저에서만 적용 되고, false이거나 지원되지 않을 경우 wrapper의 margin을 사용.
	//webkit에서는 좀 병맛이어서 margin 대신 최소 translate를 사용(true이면 translate3d를 사용).
	//css transform을 사용할 경우 환경에 따라 다소 느려질 수 있음.
	usetransform : false
...
fs.set=function(target, option, preset){
	...
	//animate, useswipe, tag, slidermode 속성은 x, y 구분하지 않고 공통으로 사용.
	var scrollbar={
		id : items.length,
		target : target,
	...
	//wrapper에 적용할 css 속성. webkit은 margin으로 하면 좀 병맛이어서 최소 translate로 강제 지정
	...
	for(i=0; i<2; i++){
		...
		//아닐 경우 defaultoption에 해당하는 값을 scrollbar변수에 추가
			...
			//animate, useswipe, tag속성은 공통으로 사용하기 위해 x, y 별도 옵션에서는 삭제
			...
			if(!scrollbar[flag].hide){
				...
				if(flag=='x'){
					...
					scrollbar[flag].bsizem=scrollbar[flag].bar.offsetHeight-12; // 스크롤바 크기수정
				}
...
fs.reset=function(scrollbar, notdisplayed){
	...
	for(i=0; i<2; i++){
		...
		if(currentscroll.show){
			...
			//wrapper에 css가 주어진 후 다시 contentsize를 구함
			currentscroll.contentsize=Math.max(target[flags[i][5]], wrapper[flags[i][4]]);
...
//page set. 현재 page와 다르면 pagechange event 호출
//page가 변경되었는지 아닌지 true | false return
