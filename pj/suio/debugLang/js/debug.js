...
$.fn.eDebug = function(opts){
    ...
    var objDebug = {
        ...
        eTableCheck : function(){
            if(flagHost){
                    ...
                    if(!findBoard.hasClass('typeHead')){
                        if(figureLength > 1){
                            ...
                            if(!findTable.hasClass('eChkTr') && !findTable.hasClass('eChkColor')){
                                findBoard.css({'position':'relative'}).append('<div class="dTableWrap"><span class="dTableMsg">전체 체크 기능: table 태그에 "eChkTr" 혹은 "eChkColor" 클래스를 넣어주세요.</span></div>');
                                figureCount++;
                            }
                        }
                    }else{
                ...
                $('table[class*=eChk]').each(function(){
                    var findParent = $(this).parents('.mBoard:first');
                    if(!findParent.hasClass('typeBody')){
                        if($(this).find('.allChk').length <= 0){
                            findParent.css({'position':'relative'}).append('<div class="dTableWrap"><span class="dTableMsg">표안에 checkbox가 없습니다. &lt;table class="eChkXX"&gt; 를 제거하세요</span></div>');
                            figureCount++;
                        }
                    }
                });
        ...
        ePlaceholder : function(){
            var figureCount=0;
            $('*[placeholder]').each(function(i){
                var propWidth = $(this).attr('style');
                if ( typeof(propWidth) != 'undefined'){
                    propWidth = propWidth.replace(' ', '');
                }
                var reg = new RegExp('[0-9]%','gi');
                if(reg.test(propWidth) == true){
                    $(this).wrap('<div class="placeholderPercent" />');
                    figureCount++;
                }
            });
            objDebug.eReport('placeholder style width 지정', figureCount);
        },
        ...
        eIntroduceArea : function (){
            ...
            // recommend 추천 부가서비스의 service 갯수와 paginate 갯수가 다를 경우
...
;(function (global, $) {
    ...
    function eDebugBegin(opts){
        ...
        $.getScript("/suio/debugLang/js/debugbot_ui.js", function() {    //디버깅 ui js
            $.fn.eDebug(opts);
        });
    }
    ...
    $.fn.eDebugBot.arrDefaults = {
        ...
        ePlaceholder : true,  // placeholder style="width" 누락한 경우 디버그
