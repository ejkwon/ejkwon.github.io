    ...
    $.fn.fixResize = function (method) {
            ...
            setup: function () {
                ...
                if(listView){
                    ...
                    if(settings.colResize){
                        ...
                        if(settings.bgSpace){
                            $wrapper.find('.grid-thead .grid-table').css('width', tableWidth);
        ...
        var helpers = {
            ...
            _setupFixedColumn: function ($self, obj, $wrapper, $divHead, captionName) {
                ...
                firstTdChildrenSelector = 'tbody tr > *:not(:nth-child(n+' + (settings.fixColumn + 1) + '))';
                ...
                $tbody.appendTo($fixedColumn).css({ 'height': fixedBodyHeight + Math.round(tableProps) });
                ...
                var culArr = [];
                $fixedColumn.find('tbody tr').each(function(index) {
                    var classChk = $(this).parents($fixedColumn).next().find('tbody tr').eq(index).attr('class');
                    if(classChk){
                    ...
                    $(this).children().each(function(i){
                        ...
                        if(rowChkNum > 0){
                            ...
                            for(var n = (index+1); n <= (index+(rowChkNum-1)); n++){
                                ...
                                var cul = (n*settings.fixColumn)+i;
                                culArr.push(cul);
            ...
            _reSetFixedColumn: function ($self, $wrapper, $divHead, index, str) {
                ...
                function fixcolumWidthEvent(n){
                    if(chrome){
                        var fixBodyWidthNum = $fixedBody.find(firstTdChildrenSelector).eq(n).outerWidth(true)-Math.round(tableProps);
                        $fixedColumn.find('thead th').eq(n).css({'width':fixBodyWidthNum});

                        fixedColumnWidth += (fixBodyWidthNum+Math.round(tableProps));
                    }else{
                ...
                $fixedColumn.find('tbody tr').each(function(i) {
                    for(var j = 0; j<settings.fixColumn; j++){
                        var num = (i*settings.fixColumn)+j,
                            target = $fixedColumn.find('tbody tr:eq('+i+') td:eq('+j+')'),
                            tdwidth = tdHeadArr[num],
                            len = $fixedColumn.find('tbody tr:eq('+i+') td').length;
                        ...
                        target.css({
                            'width': (tdBodyArr[num]-Math.round(tableProps)),
                            'height': tdwidth
                        });
                    }
                });
            ...
            _getTableProps: function($obj) {
                var tablePropNum = 1;//0;
                /*borderCollapse = 1;
                 if (settings.borderCollapse == true) {
                 borderCollapse = 2;
                 }
                 tablePropNum = ($obj.find('th:first-child').outerWidth() - $obj.find('th:first-child').innerWidth()) / borderCollapse;*/
            ...
            _setupClone: function($wrapper, $obj, str, index, tstr) {
                ...
                if(settings.bgSpace && tstr != "view"){
                    ...
                    var tableWidth = colBodyTarget.width()-scrolllen;
                    if(orgNowNum > trnWidth){
                        tableWidth = colBodyTarget.find('.grid-table').width()-scrolllen;
                    }
                        ...
                        if(j == 0){
                            ...
                            if(arr.length > 1){
                                ...
                                if(chkNum.length > 1){
                                    ...
                                    var targetStr = tagChkArr[(tagChkArr.length-1)].trim();
                                    var len = targetStr.length;
                                    if(len > 0){
                                        var name = theadName.split(targetStr);
                                        var nameStr = name[0].trim();
                                        if(settings.colResize){
                                            target.append( '<span class="resizer" title="마우스 드래그로 컬럼의 넓이를 변경 할 수 있습니다."></span><span class="text">'+nameStr+'</span> '+targetStr );
                                        }else{
                                            target.append( '<span class="text">'+nameStr+'</span>'+targetStr );
                                        }
                                    }else{
                                        if(settings.colResize){
                                    ...
                                    if(settings.colResize){
                                        target.append( '<span class="resizer" title="마우스 드래그로 컬럼의 넓이를 변경 할 수 있습니다."></span><span class="text">'+arr[0]+'</span> '+str );
                                    }else{
            ...
            _eventHandler: function($self, $wrapper, $divHead, $divBody) {
                ...
                if(settings.colResize){
                    ...
                    function thClickEvent($this){
                        ...
                        if(index < (len-divNum)){
                            ...
                            startWidth = $handle.innerWidth()+Math.round(tableProps);
                            secondWidth = $handle.next().innerWidth()+Math.round(tableProps);
                            if(settings.dragVer == 2){
                ...
                /*-------------------------------------------------------------------------------------------------------*/
                //layout
                $('body').delegate('.snbToggle, .eFoldedSnb', 'click', function(event) {
                    if(settings.bgSpace){
            ...
            _resizeCallEvent: function($self, $wrapper, $divHead, str){
                ...
                for(var i = 0; i<colen; i++){
                    ...
                    if(str == "S"){
                        ...
                        var startWidth = $handle.innerWidth()+Math.round(tableProps);
                        var secondWidth = $handle.next().innerWidth()+Math.round(tableProps);
            ...
            _mouseMoveEvent: function($self, $wrapper, $divHead, $handle, startWidth, secondWidth, startX, eventPageX, str, i){
                ...
                if(str != "S"){
                    ...
                    if(str != "L" && bol){
                        var firstWidth = (startWidth + (eventPageX - startX))+Math.round(tableProps);
                        ...
                        if(firstWidth < minWidth){
                            ...
                            if(settings.resizeVer == "fix"){
                                var nxWidth = (secondWidth - (eventPageX - startX))+Math.round(tableProps);
                                if(nxWidth < minWidth){
                                    nxWidth = minWidth;
                                }
                        ...
                        if(firstWidth >= minWidth){
                            ...
                            if(settings.bgSpace && index >= (colHeadTarget.find('thead th').length-1) || index <= 0){
                                firstWidth -= Math.round(tableProps);
                                minWidth -= Math.round(tableProps);
                                rememberNum -= Math.round(tableProps);
                                if(index <= 0){
                            ...
                            if(settings.bgSpace){
                                ...
                                colHeadTarget.css('width' , lastTwidth);
                ...
                if(str != "S"){
                    if(settings.resizeVer == "fix" && str == "M"){
                        var tn = index+1;
                        var nextWidth = (secondWidth - (eventPageX - startX))+Math.round(tableProps);
            ...
            _mouseEvent: function($wrapper){
                ...
                findFixedTarget.delegate('.eChkColor .rowChk', 'click', function(){
                    ...
                    var findTbodyTarget = findfixbodyTarget.find('tbody tr').eq(figureNum);
                    var findRowChkTarget = findTbodyTarget.find('.rowChk');
                    if($(this).is(':checked')){
                        findRowChkTarget.prop('checked', true);
                        findTbodyTarget.addClass('highlight');
                    } else {
                        findRowChkTarget.prop('checked', false);
                        findTbodyTarget.removeClass('highlight');
                    }
                ...
                findFixedTarget.delegate('.allChk', 'click', function(){
                    ...
                    if(findTable.hasClass('eChkColor')){
                        ...
                        var findNoRowChk = findfixbodyTarget.find('tbody tr').each(function(i){
                            if(!$(this).children().children().hasClass('rowChk')){
                                if(findThis.is(':checked')){
                                    if(settings.fixColumn > 0){
                                        findfixcolTarget.find('tbody tr').eq(i).addClass('highlight');
                                    }
                                    findfixbodyTarget.find('tbody tr').eq(i).addClass('highlight');
                                }else{
                                    if(settings.fixColumn > 0){
                                        findfixcolTarget.find('tbody tr').eq(i).removeClass('highlight');
                                    }
                                    findfixbodyTarget.find('tbody tr').eq(i).removeClass('highlight');
                                }
                            }
                        });
                    }
                ...
                function chkTrHover(findTarget, findFixedTarget, figureNum){
                    ...
                    if(findRowspan > 0){
                        ...
                        /*var reNum = figureNum-1;
                         if(reNum <= 0){reNum = 0;}
                         var findPrevTarget = findfixbodyTarget.find('tbody tr').eq(reNum);
                         findTdSelected(findPrevTarget, 1);*/
                    }
                ...
                function findTdSelected(findTarget, findRowspan){
                    ...
                    for(var i = 0; i<len; i++){
                        ...
                        if(!num){ num = 0; }
                        if(findRowspan < num){
                            var bol = findTarget.children().eq(i).hasClass('highlight');
                            if(bol){
                                findTarget.children().eq(i).removeClass('highlight');
                            }else{
                                findTarget.children().eq(i).addClass('highlight');
                            }
                        }
                    }
                }
                ...
                function tableTrHover(findTarget, findFixedTarget, str){
                    ...
                    var findNoRowspan = findfixbodyTarget.find('tbody tr').each(function(i){
                        ...
                        if(figureNum >= 1){
                            ...
                            if(figureindex >= figurei && figureindex < (figurei + findRowspan)){
                                for(var j = figurei; j< (figurei + findRowspan) ; j++){
                                    if(str == "over"){
                                        if(settings.fixColumn > 0){
                                            findfixcolTarget.find('tbody tr').eq(j).addClass('hover');
                                        }
                                        findfixbodyTarget.find('tbody tr').eq(j).addClass('hover');
                                    }else{
                                        if(settings.fixColumn > 0){
                                            findfixcolTarget.find('tbody tr').eq(j).removeClass('hover');
                                        }
                                        findfixbodyTarget.find('tbody tr').eq(j).removeClass('hover');
                                    }
                                }
                            }else{
                                if(str == "over"){
                                    if(settings.fixColumn > 0){
                                        findfixcolTarget.find('tbody tr').eq(figureindex).addClass('hover');
                                    }
                                    findfixbodyTarget.find('tbody tr').eq(figureindex).addClass('hover');
                                }else{
                                    if(settings.fixColumn > 0){
                                        findfixcolTarget.find('tbody tr').eq(figureindex).removeClass('hover');
                                    }
                                    findfixbodyTarget.find('tbody tr').eq(figureindex).removeClass('hover');
                                }
                            }
                        }
                    });
